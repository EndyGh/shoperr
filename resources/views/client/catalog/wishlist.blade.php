        ...
        @if (sizeof(Cart::instance('wishlist')->content()) > 0)
            ...
            <table class="table">
                ...
                @foreach (Cart::instance('wishlist')->content() as $item)
                    <tr>
                        <td class="table-image"><a href="{{ url('/', [$item->model->slug]) }}"><img src="{{ asset('img/' . $item->model->image) }}" alt="product" class="img-responsive cart-image"></a></td>
                        <td><a href="{{ url('/', [$item->model->slug]) }}">{{ $item->name }}</a></td>
