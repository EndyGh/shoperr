            ...
            <div class="search-area">
                <form>
                    <div class="control-group">
                        <input class="search-field" placeholder="Найти товар">

                        <ul class="categories-filter animate-dropdown">
                            <li class="dropdown">

                                <a class="dropdown-toggle" @if($shop_categories_all->count())data-toggle="dropdown"@endif href="#">Все категории</a>

                                <ul class="dropdown-menu" role="menu">
                                    @foreach($shop_categories_all as $category)
                                        <li role="presentation">  <a role="menuitem" tabindex="-1" href="{{$category->getUrl()}}">{{$category->name}}</a></li>
                                    @endforeach
                                </ul>
                            </li>
                        </ul>

                        <a class="search-button" href="#" style="background-color: #3498db;"></a>
                    ...
                    <div class="basket">
                        <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                            <div class="basket-item-count">
                                ...
                                <img src="https://transvelo.github.io/mediacenter-html/assets/images/icon-cart.png" alt="">
                            </div>

                            <div class="total-price-basket">
                                <span class="lbl">Ваша корзина:</span>
                                            <span class="total-price">
                                                <span class="sign"></span><span class="value">{{Cart::instance('default')->total()}} <span style="text-transform:none;">грн</span></span>
                                            </span>
                            </div>
                        </a>
                        ...
                        <ul class="dropdown-menu">
                            <!-- basket-item -->
                            <?php $cart_has_content = sizeof(Cart::content()) > 0; ?>
                             @if ($cart_has_content)
                                    ...
                                    <li>
                                        <div class="basket-item">
                                            <div class="row">
                                                ...
                                                <div class="col-xs-8 col-sm-8 no-margin basket-padding">
                                                    <div class="title">{{$item->name}}</div>
                                                    <div class="price">{{$item->options->price_uah}} грн</div>
                                                </div>
                                            </div>
                                            <a href="#" class="close-btn" data-target="{{$item->rowId}}"></a>
                                            <form action="{{ url('cart', [$item->rowId]) }}" data-id="{{$item->rowId}}"
                                                  method="POST" class="side-by-side delete" style="display: none;">
                                                {!! csrf_field() !!}
                            ...
                            <li class="checkout">
                                <div class="basket-item">
                                    <div class="row">
                                        <div class="col-xs-12 <?php if(!$cart_has_content) echo "col-sm-12 text-center"; else echo "col-sm-6";?>">
                                            <a href="{{route('cart.index')}}" class="le-button inverse">Смотреть</a>
                                        </div>
                                        @if ($cart_has_content)
                                        <div class="col-xs-12 col-sm-6">
                                            <a href="{{route('checkout.index')}}" class="le-button">Оформить</a>
                                        </div>
                                        @endif
                                    </div>
                                </div>
